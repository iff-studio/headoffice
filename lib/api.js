...
const QUERIES = {
    'post': `limit: 40, order: [sys_firstPublishedAt_DESC]`,
    'model': `limit: 100, order: [sys_firstPublishedAt_DESC]`,
...
function getQuery (key) {
    return `query { ${key}Collection (${QUERIES[key]}){ items { ${SCHEMAS[key]} } } }`
}

export async function getAllByType (key, preview = false) {

    if (process.env.DISABLE_CACHE === 'true') {
        let res = await fetchGraphQL(getQuery(key))
