...
export async function getPostModelsSlugs (post) {
    var res = await fetchGraphQL(`
    query {
        postCollection(limit: 1, where: { slug: "${post}" }) {
            items {
                modelsCollection {
                    items {
                        slug
    ...
    `)
    console.log(res.postCollection);

    res = (res.postCollection?.items[0]?.modelsCollection?.items ?? []).map(function (model) {
